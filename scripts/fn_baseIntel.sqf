// =========================================================================
// RVG Base Intel Diary Updater
// =========================================================================

params ["_status", "_locationName", "_basePos"];

// -------------------------------------------------------------------------
// LOAD / RESET
// -------------------------------------------------------------------------

if (
    _status isEqualTo "LOAD" ||
    _status isEqualTo "RESET"
) then {

    player removeDiarySubject "RVG_FieldIntel";

    player createDiarySubject [
        "RVG_FieldIntel",
        "Field Intel"
    ];

    player setVariable [
        "RVG_intelSubjectCreated",
        true
    ];

    if (_status isEqualTo "LOAD") then {

        private _title = format [
            "Renegade Camp — %1",
            _locationName
        ];

        private _body = format [
            "<font color='#FF4444'>STATUS: ACTIVE</font><br/>" +
            "<font color='#FFAA00'>Location:</font> %1<br/><br/>" +
            "Clear the camp for supplies. Reinforcements will arrive if you attack.",
            _locationName
        ];

        player createDiaryRecord [
            "RVG_FieldIntel",
            [_title, _body]
        ];
    };

} else {

    // ---------------------------------------------------------------------
    // Normal SPAWN / CLEAR behavior
    // ---------------------------------------------------------------------

    if (isNil {player getVariable "RVG_intelSubjectCreated"}) then {

        player createDiarySubject [
            "RVG_FieldIntel",
            "Field Intel"
        ];

        player setVariable [
            "RVG_intelSubjectCreated",
            true
        ];
    };

    private _title = "";
    private _body  = "";

    switch (_status) do {

        case "SPAWN": {

            _title = format [
                "Renegade Camp — %1",
                _locationName
            ];

            _body = format [
                "<font color='#FF4444'>STATUS: ACTIVE</font><br/>" +
                "<font color='#FFAA00'>Location:</font> %1<br/><br/>" +
                "Clear the camp for supplies. Reinforcements will arrive if you attack.",
                _locationName
            ];
        };

        case "CLEAR": {

            _title = format [
                "Renegade Camp — %1 (CLEARED)",
                _locationName
            ];

            _body = format [
                "<font color='#44FF44'>STATUS: CLEARED</font><br/>" +
                "<font color='#FFAA00'>Location:</font> %1<br/><br/>" +
                "Supplies remain at the site. Wait for the next camp to spawn.",
                _locationName
            ];
        };
    };

    if (_title != "") then {

        player createDiaryRecord [
            "RVG_FieldIntel",
            [_title, _body]
        ];
    };
};