...
// from the base at a random bearing, then sends them to the base. Players
// engaged at the base will see hostiles approaching from an unexpected
// direction.
//
// These units are NOT tracked in RVG_renegadeGarrison — they don't count
// toward base clear. The base can clear while reinforcements are still
// alive, creating a "we thought it was over" moment.
// =========================================================================
...
while { _attempts < 10 && (_spawnPos isEqualTo []) } do {
    ...
    private _nearBuildings = nearestTerrainObjects [_testPos, ["BUILDING"], 10];
...
// Fallback if no clear spot found — just use the last attempted bearing.
if (_spawnPos isEqualTo []) then {
    _spawnPos = _basePos getPos [150, random 360];
    diag_log "=== RVG WorldGenerator: Reinforcement spawn — using fallback position ===";
};
...
// ---- Local safe spawn helper (same pattern as garrison/roamer) --------
...
// ---- Spawn 5 units -----------------------------------------------------
{
    private _offset = _spawnPos getPos [5 + (_forEachIndex * 1.5), (_forEachIndex * 72) + _bearing];
    [_grp, _x, _offset, "RVG_reinforcementUnit"] call _fnc_safeSpawn;
} forEach _classes;

// ---- Waypoints — push to base, then engage -----------------------------
private _wpApproach = _grp addWaypoint [_basePos, 0];
...
private _wpClear = _grp addWaypoint [_basePos, 0];
...
_wpClear setWaypointTimeout [300, 600, 900];
...
[_grp] spawn {
    ...
    if (!isNull _grp) then {
        ...
        deleteGroup _grp;
        diag_log "=== RVG WorldGenerator: Reinforcements timed out ===";
    };
};
