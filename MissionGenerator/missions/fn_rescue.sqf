...
private _fnc_removeSlot = {
    ...
    if !([] call _fnc_isCurrentMission) exitWith {
        diag_log format [
            "RVG Rescue: Slot %1 instance %2 is stale. Cleanup skipped.",
            _slot + 1,
            _missionInstance
        ];
    };
...
for "_i" from 0 to 1 do {
    ...
    private _guardPos = _spawnPos getPos [
        5 + (_i * 3),
        (_i * 180) + random 45
    ];
...
private _holdWp = _campGrp addWaypoint [
    _spawnPos,
    0
];
...
[
    format [
        "RESCUE MISSION: Survivor reported near %1.",
        _locationName
    ]
...
if !([] call _fnc_isCurrentMission) exitWith {
    ...
    diag_log format [
        "RVG Rescue: Old instance %1 detected for slot %2. Exiting.",
        _missionInstance,
        _slot + 1
    ];
};
...
if (
    ...
    diag_log format [
        "RVG Rescue: Survivor lost for slot %1.",
        _slot + 1
    ];

    [
        format [
            "RESCUE FAILED: Survivor at %1 was lost.",
            _locationName
        ]
    ] remoteExec ["systemChat", 0];
    ...
    if (!isNull _campGrp) then {
        deleteGroup _campGrp;
    };
...
[
    "SURVIVOR FOUND: Escort them back to HQ."
] remoteExec ["systemChat", 0];
...

{
    if (
        alive _x &&
        { _x distance2D _survivor < 15 }
    ) exitWith {
        _rescuer = _x;
    };
} forEach allPlayers;

// ---- Ownership check before further cleanup ----------------------------
if !([] call _fnc_isCurrentMission) exitWith {

    diag_log format [
        "RVG Rescue: Old instance %1 detected before escort.",
        _missionInstance
    ];
};
...
if (isNull _rescuer) exitWith {
    ...
    diag_log format [
        "RVG Rescue: No rescuer in range for slot %1 — aborting escort.",
        _slot + 1
    ];

    deleteMarker _markerName;

    if (!isNull _survivor) then {
...
// Delete guards now that the survivor is safe.
...
if !([] call _fnc_isCurrentMission) exitWith {
    ...
    diag_log format [
        "RVG Rescue: Old instance %1 detected during escort.",
        _missionInstance
    ];
};
...
if (
    ...
    [
        format [
            "RESCUE FAILED: Survivor from %1 was lost en route.",
            _locationName
        ]
    ] remoteExec ["systemChat", 0];
    ...
    [
        format [
            "RESCUE COMPLETE: %1 survivor recovered.",
            _locationName
        ]
    ] remoteExec ["systemChat", 0];
